...
function changeEntry(date, type, item, note = "") {
  return {
    date,
    category: "human",
    type,
...
function uniqueChanges(items = []) {
  ...
  for (const item of items) {
    const key = `${item.date}:${item.type}:${item.id}:${item.note || ""}`;
...
async function liveHumanChanges(days = DEFAULT_RECENT_DAYS) {
  ...
  const added = addedRows
    ...
    .map((item) => changeEntry(compactDate(item.permitDate), "added", item, "허가일 기준 최근 등록"));
  ...
  const removed = removedRows
    ...
    .map((item) => changeEntry(compactDate(item.cancelDate) || compactDate(item.permitDate) || end, "removed", item, "취소/취하일자 기준 최근 변동"));
...
async function changesCsv(category = "human") {
  const payload = await changesForCategory(category);
  const rows = [
