...
async function fetchMfdsText(url, retries = 2, timeoutMs = 15000, options = {}) {
  ...
  const fallbackTimeoutMs = Math.min(
    Math.max(Number(options.fallbackTimeoutMs) || timeoutMs, 1000),
    timeoutMs
  );
  let lastError;
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      if (fallbackOnFetchError && isRetriableFetchError(error)) {
        try {
          return await httpsTextRequest(url, fallbackTimeoutMs);
        } catch (fallbackError) {
...
function mergeListDetail(item, detail) {
  return {
    ...
    performanceChecked: Boolean(detail.performance?.rows?.length || detail.performanceChecked === true || item.performanceChecked === true),
    performanceParseWarning: detail.performanceParseWarning || item.performanceParseWarning || "",
...
function hasHumanClientFilters(filters = {}) {
  const hasExportFilter = filters.exportMode === "exclude" || filters.exportMode === "only";
  return Boolean(
...
async function searchMfdsWithClientFilters(query, page, cacheKey) {
  ...
  const hasExportFilter = filters.exportMode === "exclude" || filters.exportMode === "only";
  const completeListScan = Boolean(hasExportFilter || filters.performanceKinds?.length);
  const requestedFullScanLimit = Number(valueOf(query.maxClientFilterPages) || 40);
  const fullScanPageLimit = Math.min(Math.max(requestedFullScanLimit, 1), 100);
  const nativeQuery = {
    ...stripDerivedSearchFields(query),
    timeoutMs: valueOf(query.timeoutMs) || (completeListScan ? "6500" : "10000"),
    retries: valueOf(query.retries) || (completeListScan ? "1" : "2"),
    fastFail: valueOf(query.fastFail) || (completeListScan ? "1" : "0")
  };
  ...
  const maxScanPages = completeListScan
    ? Math.min(nativeTotalPages, fullScanPageLimit)
    : Math.min(Math.max(requestedScanPages, page), 25);
  const candidates = await collectSearchPages(nativeQuery, firstPage, maxScanPages, completeListScan ? 10 : 3);
...
function performanceTypeFromText(value) {
  const text = String(value || "").replace(/\s+/g, "");
  if (text.includes("수입실적")) return "수입실적";
  if (text.includes("생산실적")) return "생산실적";
  return "";
}

function performanceUnitFromText(value, type) {
  const text = cleanText(value);
  const escapedType = String(type || "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const typeUnit = escapedType
    ? text.match(new RegExp(`${escapedType}\\s*\\(\\s*(단위\\s*[:：]\\s*[^)]+)\\)`, "i"))
    : null;
  const generalUnit = text.match(/단위\s*[:：]\s*([^\s,)]+)/i);
  const unit = typeUnit?.[1] || (generalUnit?.[1] ? `단위 : ${generalUnit[1].trim()}` : "");
  return unit.replace(/\s+/g, " ").trim();
}

function normalizedPerformanceYear(value) {
  const match = String(value || "").replace(/\s+/g, "").match(/^(19|20)\d{2}(?:년)?$/);
  return match ? match[0].replace(/년$/, "") : "";
}

function normalizedPerformanceAmount(value) {
...
function parsePerformance(sectionHtml) {
  ...
  const sectionType = performanceTypeFromText(sectionText);
  if (!sectionType) return null;

  const rowsByYear = new Map();
  for (const table of parseTables(sectionHtml)) {
    const tableText = table.flat().join(" ");
    const tableType = performanceTypeFromText(tableText) || sectionType;
    if (!tableType) continue;

    let headerYearIndex = -1;
    let headerAmountIndex = -1;
    for (const row of table) {
      if (row.some((cell) => normalizedPerformanceYear(cell))) continue;
      row.forEach((cell, index) => {
        const normalized = String(cell || "").replace(/\s+/g, "");
        if (headerYearIndex < 0 && /^(?:연도|년도)$/.test(normalized)) headerYearIndex = index;
        if (headerAmountIndex < 0 && performanceTypeFromText(cell) === tableType) headerAmountIndex = index;
      });
    }

    for (const row of table) {
      const yearIndex = headerYearIndex >= 0 && normalizedPerformanceYear(row[headerYearIndex])
        ? headerYearIndex
        : row.findIndex((cell) => normalizedPerformanceYear(cell));
      if (yearIndex < 0) continue;
      const year = normalizedPerformanceYear(row[yearIndex]);
      const amountCandidates = [];
      if (headerAmountIndex >= 0 && headerAmountIndex !== yearIndex) {
        amountCandidates.push(row[headerAmountIndex]);
      }
      row.slice(yearIndex + 1).forEach((cell, offset) => {
        if (yearIndex + 1 + offset !== headerAmountIndex) amountCandidates.push(cell);
      });
      const normalizedAmounts = amountCandidates.map(normalizedPerformanceAmount).filter((value) => value !== "");
      const amount = normalizedAmounts.find((value) => value !== "-") || normalizedAmounts.find((value) => value === "-") || "";
      if (!year || !amount) continue;
      const current = rowsByYear.get(year);
      if (!current || current.amount === "-") rowsByYear.set(year, { year, amount });
    }
  }

  const rows = Array.from(rowsByYear.values()).sort((a, b) => Number(b.year) - Number(a.year));
  return rows.length
    ? { type: sectionType, unit: performanceUnitFromText(sectionHtml, sectionType), rows }
    : null;
}
...
function extractPerformanceSection(html) {
  ...
  while ((match = sectionIdRe.exec(html))) {
    ...
    if (performanceTypeFromText(text)) return section;
  }
  return "";
}
...
function parseDetailHtml(html, sourceUrl = "") {
  ...
  const performance = parsePerformance(performanceSection || extraSection);
  const performanceText = cleanText(performanceSection);
  const performanceSectionPresent = Boolean(performanceTypeFromText(performanceText));
  const performanceHasYear = /\b(?:19|20)\d{2}\b/.test(performanceText);
  const performanceChecked = Boolean(performance?.rows?.length || !performanceSectionPresent || !performanceHasYear);
  ...
  return {
    ...
    performance,
    performanceChecked,
    performanceParseWarning: performanceChecked ? "" : "실적 표에 연도 값이 있으나 금액을 해석하지 못했습니다.",
    sourceUrl,
...
async function fetchSearchPage(query, page) {
  ...
  const { text } = await fetchMfdsText(url, retries, timeoutMs, {
    fallbackOnFetchError: !fastFail,
    fallbackTimeoutMs: valueOf(query.fallbackTimeoutMs) || timeoutMs
  });
...
async function searchMfdsByContractManufacturer(query, page, cacheKey) {
  ...
  const searchStartedAt = Date.now();
  let budgetMs = Math.max(Number(valueOf(query.contractBudgetMs) || 8000), 3500);
  const requestedSort = valueOf(query.sort);
  // ponytail: cap the upstream search path so one Vercel request cannot spend its full budget on retries.
  const nativeTimeoutMs = Math.min(Math.max(Number(valueOf(query.timeoutMs) || 6500), 3000), 6500);
  const nativeQuery = {
    ...stripDerivedSearchFields(query),
    sort: requestedSort || "ITEM_PERMIT_DATE",
    sortOrder: requestedSort ? valueOf(query.sortOrder) || "false" : "true",
    timeoutMs: String(nativeTimeoutMs),
    retries: "1",
    fastFail: "0",
    fallbackTimeoutMs: String(Math.min(nativeTimeoutMs, 4500))
  };
  ...
  const nativeTotalPages = firstPage.parsed.total
    ...
    : 1;
  const batchPage = Math.max(Number(valueOf(query.contractBatchPage) || 0), 0);
  const batchMode = batchPage > 0;
  const canFullyScanCandidates = nativeTotalPages <= 12;
  if (canFullyScanCandidates) {
    // ponytail: bounded candidate lists are safe to inspect fully; wide searches retain the deadline.
    budgetMs = Math.max(budgetMs, 18000);
  }
  ...
  const maxScanPages = Math.min(
    Math.max(defaultScanPages, requestedScanPages || 0, page),
    25
  );
  const allCandidates = batchMode
    ? (batchPage <= nativeTotalPages
      ? (batchPage === 1 ? firstPage : await fetchSearchPage(nativeQuery, batchPage)).parsed.items
      : [])
    : await collectSearchPages(nativeQuery, firstPage, maxScanPages);
  const candidateLimitValue = valueOf(query.contractCandidateLimit);
  const candidateLimit = batchMode || canFullyScanCandidates
    ? 0
    : candidateLimitValue
  ...
  const canUseSlowDetails = candidateItems.length <= nativePageSize;
  const remainingBudget = Math.max(budgetMs - (Date.now() - searchStartedAt), 1200);
  const requestedDetailTimeoutMs = Number(valueOf(query.detailTimeoutMs) || (fastGlobal ? 2200 : 1800));
  const detailTimeoutMs = batchMode ? 5000 : Math.min(
    canUseSlowDetails ? Math.max(requestedDetailTimeoutMs, 12000) : requestedDetailTimeoutMs,
    canUseSlowDetails ? 12000 : canFullyScanCandidates ? 2500 : 3000,
    Math.max(remainingBudget - 400, 1000)
  );
  const detailOptions = {
    retries: batchMode ? 1 : canUseSlowDetails ? Math.max(Number(valueOf(query.detailRetries) || 1), 2) : Number(valueOf(query.detailRetries) || 1),
    timeoutMs: detailTimeoutMs,
    fallbackOnFetchError: !batchMode && !fastGlobal && (canUseSlowDetails || valueOf(query.detailFallback) === "1"),
    concurrency: batchMode ? 8 : Math.min(Number(valueOf(query.detailConcurrency) || 4), canFullyScanCandidates ? 8 : 4),
    deadlineAt: searchStartedAt + (batchMode ? 20000 : budgetMs)
  };
  ...
  let notice = batchMode
    ? `${CONTRACT_SEARCH_NOTICE} 원본 목록 ${Math.min(batchPage, nativeTotalPages)}/${nativeTotalPages}페이지를 확인했습니다.`
    : `${CONTRACT_SEARCH_NOTICE} 원본 목록 ${Math.min(maxScanPages, scannedTotalPages)}페이지 ${allCandidates.length}건 중 ${candidateItems.length}건의 상세정보 기준으로 확인합니다.`;
  if (!batchMode && allCandidates.length > candidateItems.length) {
    notice = `${notice} 응답 속도를 위해 먼저 ${candidateItems.length}건만 확인했습니다.`;
  }
  if (!batchMode && scannedTotalPages > maxScanPages) {
    notice = `${notice} ${requestedSort ? "" : "허가일 최신순 후보부터 확인했습니다. "}검색 범위가 넓어 일부 후보만 확인했습니다. 제품명, 업체명, 성분명 등으로 조건을 좁히면 더 정확합니다.`;
  }
  if (enriched.timedOut) {
    notice = `${notice} 일부 상세 확인은 시간이 초과되어 건너뛰었습니다. 조건을 더 좁히면 더 정확합니다.`;
  }
  if (enriched.failed) {
    notice = `${notice} 상세정보 ${enriched.failed}건을 불러오지 못해 해당 후보는 제외했습니다.`;
  }
  ...
  const pageSize = nativePageSize;
  const start = (page - 1) * pageSize;
  const pageItems = batchMode ? items : items.slice(start, start + pageSize);
  const incomplete = (!batchMode && (scannedTotalPages > maxScanPages || allCandidates.length > candidateItems.length)) || enriched.timedOut || enriched.failed > 0;
  return searchMemoryCache.set(cacheKey, {
    page,
    pageSize,
    total,
    ...
    items: pageItems,
    incomplete,
    ...(batchMode ? { sourceTotalPages: nativeTotalPages, sourcePageSize: nativePageSize, detailFailures: enriched.failed, detailTimedOut: enriched.timedOut } : {}),
    notice,
    sourceUrl
  });
}
...
async function getMfdsDetail(itemSeq, options = {}) {
  ...
  const cached = options.refresh === true ? null : detailMemoryCache.get(cacheKey);
  if (cached) return cached;
  ...
  const detail = parseDetailHtml(text, url);
  if (detail.performanceChecked === false) return detail;
  return detailMemoryCache.set(cacheKey, detail);
}
  ...
  const rows = await mapConcurrent(uniqueSeqs, concurrency, async (itemSeq) => {
    try {
      ...
      return {
        ...
        performanceChecked: detail.performanceChecked === true,
        performanceParseWarning: detail.performanceParseWarning || ""
      };
    } catch (error) {
      return {
        itemSeq,
...
async function generateMfdsCsv(query, cache = {}) {
  ...
  const fetchDetail = async (item) => {
    ...
    if (cached && hasCsvDetailFields(cached) && performanceDetailComplete(cached)) {
      return mergeKeepNonEmpty(item, cached);
    }
    let merged = cached ? mergeKeepNonEmpty(item, cached) : item;
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        merged = mergeKeepNonEmpty(merged, detail);
        if (hasCsvDetailFields(merged) && performanceDetailComplete(merged)) return merged;
      } catch (error) {
        merged = { ...merged, detailError: error?.message || "상세 조회 실패" };
      }
