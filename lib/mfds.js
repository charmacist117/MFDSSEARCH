...
async function searchMfdsByContractManufacturer(query, page, cacheKey) {
  ...
  const candidateLimit = canFullyScanCandidates
    ? 0
    : candidateLimitValue
  ...
  const detailOptions = {
    retries: canUseSlowDetails ? Math.max(Number(valueOf(query.detailRetries) || 1), 2) : Number(valueOf(query.detailRetries) || 1),
    timeoutMs: detailTimeoutMs,
    fallbackOnFetchError: !fastGlobal && (canUseSlowDetails || valueOf(query.detailFallback) === "1"),
    concurrency: Math.min(Number(valueOf(query.detailConcurrency) || 4), canFullyScanCandidates ? 8 : 4),
    deadlineAt: searchStartedAt + budgetMs
  };
  ...
  const incomplete = scannedTotalPages > maxScanPages || allCandidates.length > candidateItems.length || enriched.timedOut || enriched.failed > 0;
  return searchMemoryCache.set(cacheKey, {
    page,
    pageSize,
    total,
