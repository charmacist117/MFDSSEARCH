...
async function searchMfdsByContractManufacturer(query, page, cacheKey) {
  ...
  const nativeTotalPages = firstPage.parsed.total
    ...
    : 1;
  const hasProductNameFilter = Boolean(normalSearchValue(query.productName));
  const canFullyScanProductName = hasProductNameFilter && nativeTotalPages <= 4;
  if (canFullyScanProductName) {
    // ponytail: a narrow product-name query is small enough to inspect fully; broad queries keep the time ceiling.
  ...
  const maxScanPages = Math.min(
    canFullyScanProductName ? nativeTotalPages : Math.max(requestedScanPages || defaultScanPages, page),
    25
  );
  ...
  const candidateLimit = canFullyScanProductName
    ? 0
    : candidateLimitValue
  ...
  const detailTimeoutMs = Math.min(
    ...
    canFullyScanProductName ? 2500 : 3000,
  ...
  const detailOptions = fastGlobal
    ? {
        ...
        concurrency: Math.min(Number(valueOf(query.detailConcurrency) || 4), canFullyScanProductName ? 8 : 4),
        deadlineAt: searchStartedAt + budgetMs
      }
    : {
        retries: Number(valueOf(query.detailRetries) || 1),
