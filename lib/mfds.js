...
async function searchMfdsByContractManufacturer(query, page, cacheKey) {
  ...
  if (exportMode) {
    const exportFilteredItems = applyExportOnlyFilter(items, exportMode);
    if (exportFilteredItems.length !== items.length) {
      items = exportFilteredItems;
      total = items.length;
