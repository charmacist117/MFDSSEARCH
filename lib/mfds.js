...
function isExportOnlyItem(item = {}) {
  ...
  return /\uC218\uCD9C\uC6A9|\bexport\b/i.test(text);
}
...
async function searchMfdsWithClientFilters(query, page, cacheKey) {
  ...
  const filterLabel = hasPresenceFilter ? "#/$ 조건" : "추가 필터";
  const noticeParts = [
    `${filterLabel}은 원본 목록 ${Math.min(maxScanPages, scannedTotalPages)}페이지 ${candidates.length}건을 확인해 적용했습니다.`
  ];
