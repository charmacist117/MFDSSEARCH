...
function cleanText(html) {
  ...
  return decodeEntities(cleanHtml
...
function parseVetHtml(html, sourceUrl) {
  ...
  const total = parseTotal(html, totalPages ? totalPages * Math.max(rows.length, 10) : rows.length);
  return { total, totalPages, items: rows };
}
...
function extractKnownSections(text) {
  const headings = [
    ...
    ["저장방법", /저장\s*방법|보관\s*방법/i]
  ];
  const matches = [];
  for (const [title, pattern] of headings) {
    const match = pattern.exec(text);
    if (match) matches.push({ title, index: match.index, length: match[0].length });
  }
